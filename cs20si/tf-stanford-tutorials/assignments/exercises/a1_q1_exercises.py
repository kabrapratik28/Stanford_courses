...
x = tf.random_uniform(shape=[300])
y = tf.random_uniform(shape=[300])
boolean = tf.less(tf.reduce_mean(x-y),tf.constant(0,dtype=tf.float32))
diff = x-y
mean_squared_error = tf.reduce_mean(tf.square(diff)) 
sum_abs_val = tf.reduce_sum(tf.abs(diff))
out = tf.cond(boolean,lambda : mean_squared_error,lambda : sum_abs_val)