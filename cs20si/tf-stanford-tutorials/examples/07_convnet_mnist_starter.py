...
with tf.variable_scope('conv1') as scope:
    ...
    kernel = tf.get_variable("kernel",shape=(5,5,1,32),dtype=tf.float32,initializer=tf.truncated_normal_initializer())

    # create biases variable of dimension [32]
    # use tf.random_normal_initializer()
    
    biases = tf.get_variable("biases",shape=(32),dtype=tf.float32,initializer=tf.random_normal_initializer())

    # apply tf.nn.conv2d. strides [1, 1, 1, 1], padding is 'SAME'
    
    conv = tf.nn.conv2d(reshaped, kernel, strides=[1, 1, 1, 1],padding='SAME')
...
with tf.variable_scope('fc') as scope:
    ...
    w = tf.get_variable('w',shape=(input_features,1024),dtype=tf.float32,initializer=tf.contrib.layers.xavier_initializer())
    b = tf.get_variable('b',shape=(1024),dtype=tf.float32,initializer=tf.zeros_initializer())
...
with tf.variable_scope('softmax_linear') as scope:
    ...
    w = tf.get_variable('w',shape=(1024,10),dtype=tf.float32,initializer=tf.contrib.layers.xavier_initializer())
