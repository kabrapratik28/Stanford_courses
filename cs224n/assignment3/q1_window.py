...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for each_position in range(length_sentence):
            curr_pos_features = []
            prev_pos = each_position - window_size
            next_pos = each_position + window_size
            
            for i in range(prev_pos,next_pos+1):
                if i<0:
                    curr_pos_features.extend(start)
                elif i>=length_sentence:
                    curr_pos_features.extend(end)
                else:
                    curr_pos_features.extend(sentence[i])
            
            y = labels[each_position]
            # add features related to current word
            windowed_data.append((curr_pos_features,y))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embedding_features,shape=(-1,self.config.n_window_features * self.config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_loss_op(self, pred):
        ...
        cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,logits=pred)
